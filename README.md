...
├── RateMonitorAgent
│   ├── Tool: fred_api.py (미국 금리 — 연방기금금리, 국채 수익률)
│   ├── Tool: bok_api.py (한국 금리 — 기준금리, 콜금리, 국고채)
│   └── Service: rate_service.py (변동 분석, 수익률 곡선 판단)
├── (향후) USEarningsAgent
├── (향후) KREarningsAgent
├── (향후) CommodityTrackerAgent
├── (향후) SemiMaterialAgent
├── (향후) DevTrendsAgent
└── PublisherAgent
...
| RateMonitorAgent      | 미국/한국 금리 모니터링 + 수익률 곡선   | FRED, 한국은행     | ✅ 완료 |
...
| CommodityTrackerAgent | 원자재/에너지 선물 이슈 트래킹          | Yahoo Finance 등   | 🔜 예정 |
| SemiMaterialAgent     | 반도체 소재 지수 트래킹 (헬륨, 웨이퍼 등) | Yahoo Finance 등 | 🔜 예정 |
...
│   │   ├── fred_api.py     #   - FRED 미국 금리 데이터 수집
│   │   └── bok_api.py      #   - 한국은행 금리 데이터 수집
...
| `FRED_API_KEY`      | O    | FRED 미국 경제 데이터 API Key (금리 모니터에 필요)                  |
| `BOK_API_KEY`       | O    | 한국은행 Open API Key (금리 모니터에 필요)                          |
...
미국/한국 주요 금리를 조회하고, 변동 방향과 수익률 곡선 상태를 분석합니다.

```bash
python -m src.services.rate_service
```

### FRED 미국 금리 데이터 수집

```bash
python -m src.tools.fred_api
```

### 한국은행 금리 데이터 수집

```bash
python -m src.tools.bok_api
```
...
| `FRED_API_KEY`      | FRED 미국 경제 데이터 API Key  |
| `BOK_API_KEY`       | 한국은행 Open API Key          |
...
| 시리즈 ID | 지표                           | 설명                        |
| --------- | ------------------------------ | --------------------------- |
| `DFF`     | 연방기금금리 (Fed Funds Rate)  | Fed 통화정책 기준금리       |
| `DGS2`    | 2년물 국채 수익률              | 단기 금리 벤치마크          |
| `DGS10`   | 10년물 국채 수익률             | 장기 금리 벤치마크          |
| `DGS30`   | 30년물 국채 수익률             | 초장기 금리                 |
| `T10Y2Y`  | 10년물 - 2년물 스프레드        | 수익률 곡선 역전 감지용     |

**한국 금리 (한국은행 ECOS)**

| 통계표       | 지표              | 설명                        |
| ------------ | ----------------- | --------------------------- |
| `722Y001`    | 기준금리          | 한국은행 통화정책 금리      |
| `817Y002`    | 콜금리 (익일물)   | 초단기 시장금리             |
| `817Y002`    | CD금리 (91일)     | 단기 시장금리               |
| `817Y002`    | 국고채 3년        | 중기 국채 수익률            |
| `817Y002`    | 국고채 10년       | 장기 국채 수익률            |
...
| ----------------- | ----- | ------------ |
| > +0.01pp (1bp)   | UP    | 금리 상승    |
| < -0.01pp (-1bp)  | DOWN  | 금리 하락    |
| ±0.01pp 이내      | FLAT  | 보합         |

### 수익률 곡선 상태

10년물 - 2년물 국채 수익률 스프레드로 경기 상태를 판단합니다:

| 스프레드         | 상태    | 의미                                |
| ---------------- | ------- | ----------------------------------- |
| > +0.10pp        | 정상    | 장기 금리가 단기보다 높음 (건강)    |
| ±0.10pp 이내     | 평탄화  | 스프레드 거의 0에 근접 (주의)       |
| < 0              | 역전    | 장단기 금리 역전 — 경기침체 경고    |
...
**Step 5: 금리 모니터 (FRED, 한국은행)** - 완료

- [x] FRED API 연동 (연방기금금리, 2/10/30년물 국채, 장단기 스프레드)
- [x] 한국은행 ECOS API 연동 (기준금리, 콜금리, CD금리, 국고채 3/10년)
- [x] 주간/월간 변동 방향 자동 계산 (UP/DOWN/FLAT)
- [x] 수익률 곡선 역전 감지 (10Y-2Y 스프레드 기반)
- [x] Slack 다이제스트에 금리 섹션 통합
- [x] RateMonitorAgent (crewAI) 등록
...
| Step 7  | 원자재/에너지 선물 트래커 (WTI, 천연가스, 금, 구리 등)      | 🔜 예정 |
| Step 7.5 | 반도체 소재 지수 트래커 (헬륨, 네온, 실리콘 웨이퍼, 희토류) | 🔜 예정 |
